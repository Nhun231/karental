...
const RentalDatePicker = ({ available }) => {
    ...
    const handlePickUpChange = (newDate) => {
        ...
        setTempPickUpDate(newDate.hour(pickUpHour));
        const minDropOff = newDate.add(2, "hour");

        if (tempDropOffDate.isBefore(minDropOff)) {
            setTempDropOffDate(minDropOff);
                                            ...
                                            .filter((h) => {
                                                if (label === 'Pick-up') {
                                                    return tempPickUpDate.isSame(today, "day")
                                                        ? h >= tempPickUpDate.hour() // Nếu là hôm nay, chỉ hiển thị giờ >= tempPickUpDate.hour()
