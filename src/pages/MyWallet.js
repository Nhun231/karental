...
const MyWallet = () => {
  ...
  .map((item, index) => ({
    ...
    amount: item.amount != null ? item.amount.toLocaleString() : "N/A", // Use `??` to handle `null` or `undefined`
...
const [refresh, setRefresh] = useState(false);
  ...
  useEffect(()=>{
    ...
    try {
      ...
      if (response) {
        alert("Top-up status: ", response.data.data.status);
        console.log(response)
        console.log("Navigating to /my-wallet..."); // Debugging log
        nav("/my-wallet"); // Redirect after processing
      }
    } catch (error) {
  ...
  const handleSearchDate = async () => {
    ...
    const transactionsWithId = res.data.listTransactionResponse.map((item, index) => ({
      ...item,
      id: index + 1,
      createdAt:item.createdAt || "N/A",
      bookingNo: item.bookingNo || "N/A",
  ...
  if (!amountWithdraw || amountWithdraw <= 0) {
    ...
    const withdraw = async (formWithdraw)=>{
      try{
        ...
        if (!res || !res.data) return;
        alert( `Successfully with draw ${res.data.transactionResponse.amount} VND`)
          handleCloseWithdraw();
      }catch(error){
                ...
                <ModalDialog>
                  ...
                  <Box textAlign={"center"}>
                    ...
                    <Typography variant={"h7"}>
                      Your current balance is {balance} VND.
                    </Typography>
                    <br />
                    <Typography variant={"h7"}>
